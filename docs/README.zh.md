...
### 隐私与离线运行
- `LLM_DRIVER=mock` / `EMBEDDER=mock`（默认值）在零模型下载的情况下运行完整的摄入→检索→引用管线，可在毫秒内完成测试；真实推理则通过 `llama-cpp-python` 在 GGUF 模型上运行（Metal、CUDA 或 CPU）。
- 前端是零外部依赖的原生 HTML/CSS/JS——无 CDN script 标签，无 npm 构建步骤，无图标字体——字体与图标均以本地 `.woff2`/`.svg` 文件形式内置。
- 整个测试套件在 `pytest-socket` 阻止所有真实网络访问的情况下运行，其中包含一个正向控制测试，用于证明该阻止一旦被静默禁用就会失败。

### 确定性反幻觉
- 检索范围仅限于该对话中的文档，绝不是全局索引或模型的预训练数据。
- 当检索到的文本块中没有一个的余弦相似度超过阈值时——该阈值逐块判定，而不仅仅取最佳匹配——keepr 会在调用 LLM *之前*就拒绝作答（`src/rag/engine.py`），这是一次确定性的 `float` 比较，而不是交给模型的主观判断。
- 生成完成后，每个 `[chunk_N]` 引用都会与该轮实际检索到的文本块 ID 集合进行成员关系校验，因此伪造的引用会被拒绝而非被信任。

### 对话与文档
- 每个对话都拥有独立的检索范围——如同 Claude Project 或 NotebookLM 笔记本——并支持侧边栏搜索、置顶收藏，以及通过右键上下文菜单进行内联重命名或删除。
- 文档通过内容哈希（SHA-256）去重——重复添加同一文件是无操作，而不会产生重复的文本块集合。

### 持久化的后台工作进程
- 摄入（`IngestionWorker`）与生成（`GenerationWorker`）各自运行在独立于 HTTP 连接的、由数据库驱动的队列上，因此一次嵌入操作只会等待前一个嵌入操作，绝不会等待无关的 LLM 生成。关闭标签页、断网，或在回答生成过程中刷新页面，都不会暂停或丢失正在进行的工作——重新打开会通过 SSE 实时重新连接。
- 重启后，任何因上次崩溃而卡在处理中途的任务都会被标记为错误状态，并保留其已生成的部分内容，而不是永远空转等待。
- 每种文件类型都经过同一套 `Ingestor` 协议——每个文件的状态（`暂存中→提取中→分块中→嵌入中→已索引`）由真实的 SSE 事件驱动，而非虚假的定时器——这也是未来新增格式（例如音频/视频，目前是一个会抛出 `UnsupportedSourceError` 而非崩溃或静默无操作的占位实现 `AudioVideoIngestor`）时无需修改下游任何代码的原因。

### 内置模型管理
- 设置菜单会列出 `models/` 中的每个 `.gguf` 文件，仅根据其自身元数据（一个池化层标记）而非文件名，将其分类为 LLM 模型或嵌入模型。
- 模型可直接从 Hugging Face Hub 下载并显示实时进度；切换当前使用的模型会保存该选择并重启应用，以便新模型真正被加载——删除模型文件也以同样的方式生效。
...
### 原生桌面应用
- 一个 [Tauri](https://tauri.app/) v2 封装器将经 PyInstaller 编译的后端打包为原生安装程序——macOS 的 `.app`/`.dmg`、Windows 的 `.exe`/`.msi`、Linux 的 `.AppImage`/`.deb`——通过 CI 构建矩阵生成，目标机器上无需安装 Python。
- 同一套代码库既可以作为 Web 应用运行（`make run`，支持热重载），也可以构建为原生桌面应用（`make tauri-build`）。
